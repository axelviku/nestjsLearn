...
export class SettingService {
  ...
  async languageCoverage(languageCoverageData:{languageFrom:string,languageTo:string},appLanguage: string): Promise<object> {
    try {
      ...
      if (languageCoverageData.languageFrom != undefined && languageCoverageData.languageFrom !="" ) {
        query.languages = new mongoose.Types.ObjectId(languageCoverageData.languageFrom);
      }
      if (languageCoverageData.languageTo != undefined  && languageCoverageData.languageTo !="" ) {
        query.languages = new mongoose.Types.ObjectId(languageCoverageData.languageTo);
      }
      if (languageCoverageData.languageFrom != undefined && languageCoverageData.languageFrom !="" && languageCoverageData.languageTo != undefined && languageCoverageData.languageTo !="" ) {
        query = {
          $and: [{
            languages: new mongoose.Types.ObjectId(languageCoverageData.languageTo)
          }, {
            languages: new mongoose.Types.ObjectId(languageCoverageData.languageFrom)
          }]
        };
      }

      const coverage = await this.coverageModel.find(query)
        .select({
          languages: 1,
          coverage: 1,
          _id: 0
      ...
      await Promise.all(coverage.map(async (item) => {
        var local:any = {};
        local = item.toObject();
        let languages_new = local.languages.map((lang: any) => {
          return {
            _id: lang._id,
