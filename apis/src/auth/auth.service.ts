...
export class AuthService {
  ...
  async requestPhoneOtp(
    ...
    try {
      const twilioServiceStatus: any =
        process.env.TWILIO_VERIFY === 'true' ? true : false;
      this.logger.log(
        `/auth/requestOtp/=====> requestPhoneOtp ===> twilioServiceStatus : `,
        twilioServiceStatus,
      );

      const findUserPhoneOtp: any = await this.otpModel.findOne({
        'requestOtpType.phoneNumber': requestOtp.phone,
        otpType: 'SIGNUP',
        phoneEmailAddress: requestOtp.email,
      });
      ...
      if (
        ...
        if (currentTime >= validUntilDb) {
          ...
          if (
            findUserPhoneOtp &&
            findUserPhoneOtp.resendOtpCounter ===
              process.env.TWILIO_OTP_RESEND_DAILY_LIMIT
          ) {
            this.logger.log(
              `/auth/requestOtp/=====> requestPhoneOtp S ===> Daily Resend OTP 5 counter Limit Reached and we RESET the count ! `,
            );

            const twilioOtp: any = twilioServiceStatus
              ? await this.smsService.twilioRequestPhoneOtp(requestOtp)
              : { status: 'pending' };
            this.logger.log(
              `/auth/requestOtp/=====> requestPhoneOtp ===> 1 twilioOtp `,
              JSON.parse(JSON.stringify(twilioOtp)),
            );
            if (twilioOtp && twilioOtp.status === 'pending') {
              const updateRecord = await this.otpModel.findOneAndUpdate(
                {
                  'requestOtpType.phoneNumber': requestOtp.phone,
                  otpType: 'SIGNUP',
                  phoneEmailAddress: requestOtp.email,
                },
                {
                  ...
                  twilioReqResponse: JSON.parse(JSON.stringify(twilioOtp)),
                  deviceInfo: {
                    os: deviceInfo.os,
              ...
              return {
                ...
                info: {
                  otpsent: false,
                  count: 1,
                  isVerified: updateRecord.isVerified,
              ...
              this.logger.warn(
                `/auth/requestOtp/=====> requestPhoneOtp ===> else 1 twilioOtp `,
                JSON.parse(JSON.stringify(twilioOtp)),
              );
              return {
                success: false,
            ...
            const twilioOtp: any = twilioServiceStatus
              ...
              : { status: 'pending' };
            this.logger.log(
              `/auth/requestOtp/=====> requestPhoneOtp ===> 2 twilioOtp `,
              twilioOtp,
            );
            if (twilioOtp && twilioOtp.status === 'pending') {
              ...
              return {
                ...
                info: {
                  ...
                  isVerified: updateSeq.isVerified,
                  isUserRegister: false,
                },
                message: this.i18n.t('lang.OTP_SENT', { lang: lang }),
              };
              ...
              this.logger.warn(
                `/auth/requestOtp/=====> requestPhoneOtp ===> else 2 twilioOtp `,
                twilioOtp,
              );
              return {
                success: false,
          ...
          if (
            ...
            return {
              ...
              info: {
                ...
                isVerified: findUserPhoneOtp.isVerified,
                isUserRegister: false,
              },
            ...
            const twilioOtp: any = twilioServiceStatus
              ...
              : { status: 'pending' };
            this.logger.log(
              `/auth/requestOtp/=====> requestPhoneOtp ===> 3 twilioOtp `,
              twilioOtp,
            );
            if (twilioOtp && twilioOtp.status === 'pending') {
              const updateSeq = await this.otpModel.findOneAndUpdate(
              ...
              this.logger.warn(
                `/auth/requestOtp/=====> requestPhoneOtp ===> else 3 twilioOtp `,
                twilioOtp,
              );
              return {
                success: false,
        ...
        if (twilioOtp && twilioOtp.status === 'pending') {
          await this.otpModel.create({
            otp: twilioServiceStatus ? '000000' : '123456',
            'requestOtpType.phoneNumber': requestOtp.phone,
            otpType: 'SIGNUP',
  ...
  async verifyOTPPhone(otp: any, lang: string) {
    try {
      const twilioServiceStatus: any =
        process.env.TWILIO_VERIFY === 'true' ? true : false;
      this.logger.log(
        `/auth/requestOtp/=====> verifyOTPPhone ===> twilioServiceStatus : `,
        twilioServiceStatus,
      );

      const queryPhoneOtp: any = twilioServiceStatus
        ? {
            'requestOtpType.phoneNumber': otp.phone,
            otpType: 'SIGNUP',
            'requestOtpType.countryCode': otp.countryCode,
            phoneEmailAddress: otp.email,
          }
        : {
            otp: otp.otp,
            'requestOtpType.phoneNumber': otp.phone,
      ...
      const otpCheck = await this.otpModel.findOne(queryPhoneOtp);
      ...
      const twilioOtp: any = twilioServiceStatus
        ? await this.smsService.twilioVerifyPhoneOtp(otp)
        : { status: 'approved' };
      this.logger.log(
        `/auth/requestOtp/=====> requestPhoneOtp ===> 1 twilioOtp `,
        twilioOtp,
      ...
      if (twilioOtp && twilioOtp.success === false) {
        this.logger.warn(
          `/auth/verifyOtp/=====> verifyOTPEmail ===> OTP_EXPIRED ! `,
        );
        return {
          success: false,
          info: { status: 'expired' },
          message: this.i18n.t('lang.OTP_EXPIRED', { lang: lang }),
        };
      } else if (
        twilioOtp &&
        twilioOtp.success === true &&
        twilioOtp.info &&
        twilioOtp.info.status === 'approved'
      ) {
        await this.otpModel.updateOne(
          ...
          {
            isVerified: true,
            twilioVerifyResponse: JSON.parse(JSON.stringify(twilioOtp.info)),
          },
        );
        return {
          success: true,
        ...
        this.logger.warn(
          `/auth/requestOtp/=====> verifyOTPPhone ===> else 1 twilioOtp `,
          JSON.parse(JSON.stringify(twilioOtp)),
        );
        return {
          success: false,
  ...
  async requestEmailOtp(
    ...
    try {
      const twilioServiceStatus: any =
        process.env.TWILIO_VERIFY === 'true' ? true : false;
      this.logger.log(
        `/auth/requestOtp/=====> requestEmailOtp ===> twilioServiceStatus : `,
        twilioServiceStatus,
      );
      const findUserEmailOtp: any = await this.otpModel.findOne({
      ...
      if (findUserEmailOtp != null || findUserEmailOtp != undefined) {
        if (currentTime >= validUntilDb) {
          ...
          if (
            findUserEmailOtp &&
            findUserEmailOtp.resendOtpCounter ===
              process.env.TWILIO_OTP_RESEND_DAILY_LIMIT
          ) {
            this.logger.log(
            ...
            const twilioOtp: any = twilioServiceStatus
              ? await this.smsService.twilioRequestEmailOtp(requestOtp.email)
              : { status: 'pending' };
            this.logger.log(
              `/auth/requestOtp/=====> requestEmailOtp ===> 1 twilioOtp `,
              JSON.parse(JSON.stringify(twilioOtp)),
            );
            if (twilioOtp && twilioOtp.status === 'pending') {
              ...
              this.logger.log(
                `/auth/requestOtp/=====> requestEmailOtp ===>else 1 twilioOtp `,
                JSON.parse(JSON.stringify(twilioOtp)),
              );
              return {
                success: false,
            ...
            const twilioOtp: any = twilioServiceStatus
              ...
              : { status: 'pending' };
            this.logger.log(
              `/auth/requestOtp/=====> requestEmailOtp ===> 2 twilioOtp `,
              twilioOtp,
            );
            if (twilioOtp && twilioOtp.status === 'pending') {
              const updateSeq = await this.otpModel.findOneAndUpdate(
              ...
              this.logger.warn(
                `/auth/requestOtp/=====> requestEmailOtp ===> else 2 twilioOtp `,
                twilioOtp,
              );
              return {
                success: false,
          ...
          if (
            ...
            const twilioOtp: any = twilioServiceStatus
              ...
              : { status: 'pending' };
            this.logger.log(
              `/auth/requestOtp/=====> requestEmailOtp ===> 3 twilioOtp `,
              twilioOtp,
            );
            if (twilioOtp && twilioOtp.status === 'pending') {
              const updateSeq = await this.otpModel.findOneAndUpdate(
              ...
              this.logger.warn(
                `/auth/requestOtp/=====> requestEmailOtp ===> else 3 twilioOtp `,
                twilioOtp,
              );
              return {
                success: false,
        ...
        const twilioOtp: any = twilioServiceStatus
          ...
          : { status: 'pending' };
        this.logger.log(
          `/auth/requestOtp/=====> requestEmailOtp ===> 4 twilioOtp  `,
          twilioOtp,
        );
        if (twilioOtp && twilioOtp.status === 'pending') {
          await this.otpModel.create({
          ...
          this.logger.warn(
            `/auth/requestOtp/=====> requestEmailOtp ===> else 4 twilioOtp `,
            twilioOtp,
          );
          return {
            success: false,
      ...
      this.logger.error(
        `/auth/requestOtp/ ===> findUserEmailOtp ===> error : `,
        error,
      );
      return {
        success: false,
  ...
  async verifyOTPEmail(otp: any, lang: string) {
    try {
      const twilioServiceStatus =
        process.env.TWILIO_VERIFY === 'true' ? true : false;
      this.logger.log(
        `/auth/requestOtp/=====> verifyOTPEmail ===> twilioServiceStatus : `,
        twilioServiceStatus,
      );

      const queryEmailOtp: any = twilioServiceStatus
        ? {
      ...
      const findUserEmailOtp: any = await this.otpModel.findOne(queryEmailOtp);

      if (
        !findUserEmailOtp ||
        findUserEmailOtp == undefined ||
        findUserEmailOtp == null
      ) {
        this.logger.warn(
      ...
      const isUserRegister =
        ...
        isUserEmailAlreadyExist.status === true
          ...
          : false;
      this.logger.log(
        `/auth/requestOtp/=====>verifyOTPEmail ==> isUserRegister ===>  ${isUserRegister}`,
      );
      ...
      const twilioOtp: any = twilioServiceStatus
        ? await this.smsService.twilioVerifyEmailOtp(otp)
        : { status: 'approved' };
      this.logger.log(
        '/auth/requestOtp/=====> verifyOTPEmail ===> 1 twilioOtp ',
        JSON.parse(JSON.stringify(twilioOtp)),
      );
  ...
  async signUp(lang: string, SignUpBody: any, headers: any) {
    try {
      ...
      const lastUserNumber = await this.userModel
        .findOne({}, { 'personalInfo.userNo': 1, _id: 0 })
        .sort({ 'personalInfo.userNo': -1 });
      ...
      const data: any = {
        ...
        personalInfo: {
          userNo: lastUserNumber
            ? parseInt(lastUserNumber.personalInfo.userNo) + 1
            : 1,
          gender,
          photo,
          countryId,
      ...
      if (prefectureId && prefectureId != null && prefectureId != undefined) {
        const prefectureIds: string = prefectureId;
        data.prefectureId = new mongoose.Types.ObjectId(prefectureIds);
      }

      if (referralCode && referralCode != null && referralCode != undefined) {
        const refCode: string = referralCode;
        data.userReferralCode = new mongoose.Types.ObjectId(refCode);
      }

      if (source && source === 'social') {
        data.social = [{ type: socialType, id: socialId }];
      }
    ...
    try {
      ...
      const data: any = {
        ...
        interpreterInfo: {
          profileDetails,
          interpretationLanguages,
          expertiseList: expertiseList.map(
            (id: string) => new mongoose.Types.ObjectId(id),
          ),
          associations,
          qualification,
  ...
  async userLogout(lang: string, req: any): Promise<any> {
    try {
      ...
      return {
        success: true,
        info: {},
        message: this.i18n.t('lang.COMMON_DETAIL_SUCCESSFULLY', { lang: lang }),
      };
    } catch (error) {
  ...
  async getCommonData(userDetails:any): Promise<any> {
    try {
      ...
      return {
        success: true,
        info: data,
        message: this.i18n.t('lang.COMMON_DETAIL_SUCCESSFULLY', { lang: userDetails.appLanguage }),
      };
    } catch (error) {
      this.logger.error(`/auth/userLogout/=====> Catch C Error : `, error);
      return {
