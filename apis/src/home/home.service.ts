...
export class HomeService {
  ...
  async getRecentUser(appLanguage: string, recUser: any, allQueryParams: any) {
    try {
      this.logger.log(
            ...
            if (userDetails.interpreterInfo.interpreterRates) {
              userData.rates = userDetails.interpreterInfo.interpreterRates.map(
                (obj) => {
                  ...
                  let preferedCurr = userDetails.personalInfo.preferredCurrency;
                  return {
                    _id: obj._id,
  ...
  async interpreterData(appLanguage: string, _id: string) {
    try {
      ...
      if (users) {
       ...
       userData.language  = await this.util.dataMapperArray(languages,appLanguage);
       ...
       const settingData: any = await this.settingModel.findOne({});
        userData.metaData = {
          ...
          responseRateText: settingData.metaData.responseRateText[
            appLanguage
          ].replace('[percentage]', userDetails.interpreterInfo.responseRate),
        };
