...
export class ResourcesService {
  ...
  async getCountryList(language: string): Promise<object> {
    try {
      const getCountry = await this.countryModel.find({ isActive: true });
      this.logger.log(
      ...
      return {
        ...
        message: this.i18n.t('lang.COUNTRY', { lang: language }),
      };
    } catch (error) {
  ...
  async getCityListForCountryId(
    ...
    try {
      console.log(
        '/resources/cities/:countryId =====> req.params : ',
        countryId,
        language,
      );
      if (!countryId)
      ...
      const getCity = await this.cityModel.find({
  ...
  async getExpertiseList(language: string): Promise<object> {
    try {
      const getExpertise = await this.expertiseModel.find({ isActive: true });
      ...
      const data = await this.utils.dataMapper(getExpertise, language);
      return {
        success: true,
        info: {
          expertise: data,
  ...
  async getLanguageList(language: string): Promise<object> {
    try {
      const getLanguage = await this.languageModel.find({ isActive: true });
      ...
      const data = await this.utils.dataMapper(getLanguage, language);
      return {
        success: true,
        info: {
          language: data,
      ...
      console.error('/resources/language/ =====> catch error : ', error);
      return {
        success: false,
        info: {
          language: [],
  ...
  async getPrefectureList(language: string): Promise<object> {
    try {
      const getPrefecture = await this.prefectureModel.find({ isActive: true });
      ...
      const data = await this.utils.dataMapper(getPrefecture, language);
