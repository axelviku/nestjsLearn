  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    try {
      ...
      const user = await this.userModel
        ...
        .select(
          '_id fullName email source favUsers userReferralCode interpreterInfo status personalInfo deviceInfo roleId',
        );
      if (!user) {
        throw new UnauthorizedException();
      }
      try {
        const userObject = {
          ...
          appLanguage: await this.getLanguage(request.headers) ? await this.getLanguage(request.headers) :'en',
        };
