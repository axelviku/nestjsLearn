...
export class AgendaService implements OnModuleInit {
  ...
  async defineJobs() {
    ...
    this.agenda.define('coverage-update', async (job, done) => {
      this.logger.log('Cron Start =============> "coverage-update"');
  ...
  async defineUpdateLanguageCoverageJob() {
    const coverageArr = [];
    let roleDetails = await this.commonService.roleData('interpreter');
    const coverageData: any = await this.userModel.aggregate([
      {
        $match: {
          roleId: new mongoose.Types.ObjectId(roleDetails._id),
    ...
    for (const [key, value] of Object.entries(occurrences)) {
      ...
      coverageArr.push({
        coverage: value,
        languages: [
          new mongoose.Types.ObjectId(lang1),
          new mongoose.Types.ObjectId(lang2),
        ],
      });
    }
    ...
    this.logger.log(
      'Cron Complete Successfully =============> "coverage-update"',
      coverageArr,
    );
  }
}
